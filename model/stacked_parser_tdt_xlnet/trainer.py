...
class Trainer:
    ...
    def train(self, test_desc):
        """ The main process of training and evaluating.
            针对 GAN 做了较大调整，将CNN部分作为模型一部分进行特征抽取
        """
    ...
    def gen_batch_iter(self):
        """ instances: 训练集数据整体封装一组元组，每个元素形式为
            (encoder_inputs, decoder_inputs, pred_splits, pred_nucl, pred_rels)  一批树对应
    ...
    def parse_and_eval(self, trees2parse, save_per, type_):
        """ 解析和评估
            trees2parse: 集合中加载的 tree_obj对象，提供解析使用
            gold_trees:  内存中常驻的tree_obj对象，供评测比对使用
        """
        gold_trees = self.dev_set if type_ == "dev" else self.test_set
        self.model.eval()  # 模式修改
        self.model_xl.eval()
        trees_eval_pred = []
